...
5. a. If the release allows for a non-destructive rebuild, use the `rebuild-keep-data` target to clean and recompose services without destroying any data
   ...
   b. If the release needs a full rebuild, all services will need to be cleaned, rebuilt and recomposed and any existing data reingested or restored from backups. This is done by running
