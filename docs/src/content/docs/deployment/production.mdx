...
The following default settings in the `.env` file should be changed when deploying CanDIG in a production environment. You can also take a look at the [`example-production.env`]() and [`etc/env/production-diff-template.diff`]() files with the changes below incorporated.
