...
1. Backup your existing deployment using the steps on the page: [Backing up and restoring CanDIG data](backup-restore-candig.md). This should be done regardless of wheteher you think the deployment will destroy any data, just in case anything goes wrong. Things you will want to back up are:
    - [ingested genomic and clinical data](backup-restore-candig/#backing-up-postgres-databases)
    - [log files](backup-restore-candig/#backing-up-logs)
    - [user authorizations](backup-restore-candig/#backing-up-secrets-and-authorization-data)
    - federation details (if your node is federated with other nodes)
        <details>
...
10. Reinstate federation with other nodes
    ...
    Using the file you created in step 1 saving federation details, do the following:
