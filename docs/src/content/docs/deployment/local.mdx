...
If you want to use an existing conda installation on your local at the bottom of the [.env](https://github.com/CanDIG/CanDIGv2/blob/stable/etc/env/example.env), set `CONDA_INSTALL` to your existing conda installation path
...
On some machines (Linux/MacOS), if you get an error something like:
...
Or repeated problems when trying to build keycloak, it may be necessary to add the following to `/etc/hosts`:
