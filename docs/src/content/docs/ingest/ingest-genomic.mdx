...
## Genomic processing times
Queues will complete and genomic files will be ingested into CanDIG (appearing in the Data Portal UI) fairly quickly (~7K files in about 30 mins).  However, the indexing of files and creating the hundreds of millions of database rows is a fairly slow process (~7K files in about 6 days, or about 1K files per day).  To monitor indexing and confirm completion, use the following psql commands (assuming variant file indexing occurs at the same time as alignment file indexing):
