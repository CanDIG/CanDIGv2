...
- `analysis_date` is the date the institute received the file from the sequencing centre, if one is not provided the CanDIG software will look for a date inside the analysis file. If one is still not found the ingest is rejected
