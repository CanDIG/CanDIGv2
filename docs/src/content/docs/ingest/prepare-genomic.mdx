...
- `analysis_date` is the date the institute received the file from the sequencing centre. Analysis date is required for all variant files, if one is not provided in the genomic json, the CanDIG software will look for a date inside the file. If one is still not found the ingest is rejected for any variant files without a date. 
...
|--------------------------------------------------------------------------------------------------------|---------------------------------|---------------------|--------------------------|
| [FastQ Format](https://maq.sourceforge.net/fastq.shtml)                                                | `.fastq` `.fq.gz` `.fq` `fq.gz` | n/a                 | n/a                      |
...
| filetype                                        | expected file extension | expected `analysis_type` | expected `analysis_attribute`s     |
|-------------------------------------------------|-------------------------|--------------------------|-----------------------------------|
| Matrix with genes as rows and counts as columns | doesn't matter as long as it is a tab-delimited text file   | `sequence_annotation`    | see below                         |
