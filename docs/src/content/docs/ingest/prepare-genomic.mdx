...
2. Add the relevant volumes to the [htsget_app docker-compose file](https://github.com/CanDIG/CanDIGv2/blob/stable/lib/htsget/docker-compose.yml), [drs docker-compose file](https://github.com/CanDIG/CanDIGv2/blob/stable/lib/drs/docker-compose.yml), [rnaget docker-compose](https://github.com/CanDIG/CanDIGv2/blob/develop/lib/rnaget/docker-compose.yml) and [candig-ingest docker-compose file](https://github.com/CanDIG/CanDIGv2/blob/stable/lib/candig-ingest/docker-compose.yml)
...
Repeat the same process as above with the drs, rnaget and candig-ingest docker compose files.
...
```json
{
    "experiments": [
        ...
        {
            "program_id": "LOCAL-SYNTH_01",
            "experiment_id": "LOCAL-SEQ_0009",
            "submitter_sample_id": "LOCAL-SAMPLE_0009",
            "metadata": {
    ...
    "analyses": [
        ...
        {
            "program_id": "LOCAL-SYNTH_01",
            "analysis_id": "LOCAL-NA18537-counts",
            "main": {
                "access_method": "file:////app/htsget_server/data/files/counts.tsv",
                "name": "counts.tsv"
            },
            "metadata": {
...
- If an S3 bucket access method is provided, assuming you have properly added the S3 credentials to vault [(see `Configuring s3 authorization`above)](#a-configure-connection-to-genomic-data-sources), the service will scan the S3 bucket to ensure the relevant files are present.
...
- `analysis_sample_id` is the name that is used inside the analysis file that is used to refer to the sample/experiment. Some common ones we see are Normal, Tumour,or may be the same identifier used for the sample. If you are not sure what this is you may need to look into the bam/vcf to see what is used.
...
Transcriptome matrices can be ingested into CanDIG. Files must be in `.tsv` format.

| filetype                                        | expected file extension | expected `analysis_type` | expected `analysis_attribute`     |
|-------------------------------------------------|-------------------------|--------------------------|-----------------------------------|
| Matrix with genes as rows and counts as columns | `.tsv` `.tsv.gz`        | `sequence_annotation`    | see below                         |

As there is no standard format for transcriptomic matrices, it is important to specify the following metadata as `analysis_attribute`s so that the file can be accurately parsed

- `gene_id_column`: name of column containing gene ids
- `length_column`: name of column containing gene lengths
- `count_column`: name of column containing counts
- `norm_column`: name of column containing normalized values
- `norm_method`: normalization method used to calculate values in norm_column, one of: `tpm`, `fpkm`, `getmm`

An example of a fully populated experiment and analysis object for a transcriptome file:
    ...
    "experiments": [
        {
            ...
            "submitter_sample_id": "LOCAL-SAMPLE_0009",
            "metadata": {
                "library_description": "<free text description of the library>",
                "instrument": "Illumina MiSeq", # selected from enum
                "library_selection": "cDNA", # selected from enum
                "protocol": "<url to protocol>",
                "library_strategy": "WTS",
    ...
    "analyses": [
        {
            ...
            "metadata": {
                ...
                "analysis_attribute": {
                    "subtype": "expression_count"
                    "reference": "hg38",
                    "gene_id_column": "<name of column containing genes in tsv>",
                    "length_column": "<name of column containing gene lengths in tsv>",
                    "count_column": "<name of column containing read counts in tsv>",
                    "norm_column": "<name of column containing read counts in tsv>",
                    "norm_method": "tpm" # selected from enum
                }
            }
            "main": {
                "access_method": "file:////app/htsget_server/data/files/counts.tsv",
...
::: tip
All options for enumerated fields can be found in the [ingest schema](https://github.com/CanDIG/candigv2-ingest/blob/develop/ingest_openapi.yaml)
:::
