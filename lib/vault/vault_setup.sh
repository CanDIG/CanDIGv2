...
docker exec $vault sh -c "echo 'path \"opa/programs\" {capabilities = [\"update\", \"read\", \"create\"]}' >> ${ingest}-policy.hcl; echo 'path \"opa/programs/*\" {capabilities = [\"create\", \"update\", \"read\", \"delete\"]}' >> ${ingest}-policy.hcl; echo 'path \"opa/site_roles\" {capabilities = [\"create\", \"update\", \"read\", \"delete\"]}' >> ${ingest}-policy.hcl; echo 'path \"opa/users/*\" {capabilities = [\"create\", \"update\", \"read\", \"delete\"]}' >> ${ingest}-policy.hcl; echo 'path \"opa/pending_users\" {capabilities = [ \"update\", \"read\", \"create\"]}' >> ${ingest}-policy.hcl; echo 'path \"opa/preapproved_users\" {capabilities = [ \"update\", \"read\", \"create\"]}' >> ${ingest}-policy.hcl; echo 'path \"opa/users\" {capabilities = [\"list\"]}' >> ${ingest}-policy.hcl; vault policy write ${ingest} ${ingest}-policy.hcl"
